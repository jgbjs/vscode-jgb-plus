...
import { escapeRegExp } from './../config/DefaultConfiguration'
...
export class RegionProvider {
  ...
  public getRegions(document: vscode.TextDocument): { completedRegions: CustomRegion[], errors: string[] } {
    ...
    for (let foldDefinition of foldDefinitions) {
      ...
      var start = new RegExp(escapeRegExp(foldDefinition.foldStartRegex), "i"); //https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
      var end = new RegExp(escapeRegExp(foldDefinition.foldEndRegex), "i");
